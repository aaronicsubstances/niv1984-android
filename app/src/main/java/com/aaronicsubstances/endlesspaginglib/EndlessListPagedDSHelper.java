...
public abstract class EndlessListPagedDSHelper<T extends EndlessListItemForPagedDS> {
    ...
    public EndlessListPagedDSHelper(Class<T> itemClass, int networkPageSize, int maxPageCount) {
    ...
    public void loadPageAsync(boolean initialLoad, String dsContext, String cacheVersion,
                              ...
                              EndlessListDataSource.Callback<T> uiCallback) {
    ...
    private void completeLoadPageAsync(final int startRank, final int endRank,
                                       ...
                                       final EndlessListDataSource.Callback<T> uiCallback) {
        CallableWithAsyncContext<T> ioCallable = new CallableWithAsyncContext<T>() {
            @Override
            public EndlessListLoadResult<T> call(Object asyncContext) throws Exception {
                if (endRank < 0) {
                    return new EndlessListLoadResult<>(new ArrayList<T>());
                }
                if (firstTimeLoad) {
                    // wipe out cache.
                    daoDeleteCachedResults(asyncContext, dsContext);
                }
                List<EndlessListPagingCacheItem> cachedResults = daoGetCachedResults(asyncContext,
    ...
    private EndlessListLoadResult<T> completePageLoading(Object asyncContext,
            String dsContext, String cacheVersion, int startRank, int endRank,
            List<EndlessListPagingCacheItem> cachedResults, boolean listValid) throws Exception {
        if (cachedResults == null) {
            cachedResults = daoGetCachedResults(asyncContext,
                    dsContext, cacheVersion, startRank, endRank);
        }
        List<T> successResult = new ArrayList<>();
        for (EndlessListPagingCacheItem cacheItem : cachedResults) {
            T item = deserializePagingCacheItem(cacheItem.getValue());
            successResult.add(item);
        }
        return new EndlessListLoadResult<>(successResult, null, listValid);
    }

    private EndlessListLoadResult<T> loadPageFromNetwork(Object asyncContext, String dsContext, String cacheVersion,
                                                         NetworkCallback<T> networkCallback,
                                                         int startRank, int endRank, boolean useInAfterPosition) throws Exception {
        ...
        for (int page = startPage; page <= endPage; page++) {
            ...
            List<EndlessListPagingCacheItem> serializedItems = new ArrayList<>();
            ...
            for (int i = 0; i < pageData.size(); i++) {
                ...
                EndlessListPagingCacheItem dbRecord = createEndlessListPagingCacheItem(asyncContext,
            ...
            if (!serializedItems.isEmpty()) {
                ...
                for (EndlessListPagingCacheItem item : serializedItems) {
    ...
    protected abstract EndlessListPagingCacheItem createEndlessListPagingCacheItem(Object asyncContext,
            String dsContext, int page, int rank, Object keyFromApi, String cacheVersion,
            Date timestamp, String serialized);

    protected abstract void daoInsert(Object asyncContext, List<EndlessListPagingCacheItem> items);

    protected abstract List<EndlessListPagingCacheItem> daoGetCachedResults(Object asyncContext,
            String dsContext, String cacheVersion, int startRank, int endRank);
    ...
    public interface CallableWithAsyncContext<T> {
        EndlessListLoadResult<T> call(Object asyncContext) throws Exception;
    }
